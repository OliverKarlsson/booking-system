...
Built for the Minut take-home assignment.
...
Those five lines _are_ the entire booking rule: the per-unit scoping, the half-open
...
| 2    |                             | reads overlaps → none found _(A hasn't committed)_ |
...
file _cannot_ insert a double booking.
...
[the timezone model](#the-timezone-model)); real foreign keys; and _less_ infrastructure,
...
- `reservations.concurrency.integration.test.ts` fires **20 simultaneous** `POST`s at one
  ...
  the table. A companion case proves bookings for _different_ units all still succeed
...
repositories hold _all_ SQL and nothing else. Every query is parameterised — there is no
...
rejected your row". It does not say _whose_ booking was in the way, and the reservation
form needs to say _"Conflicts with Jane Doe (12–15 March 2026)"_. So every write path does
both, in this order:
...
`createdAt` / `updatedAt` are the deliberate exception — those genuinely _are_ instants, so
they are `timestamptz` and serialize as ISO 8601. _When a row was written_ is a moment;
_when a guest arrives_ is a date on a calendar. Conflating the two is the original error.
...
The single exception is a _conversion_, not a comparison: the dashboard's occupied/vacant
...
Without it a Los Angeles flat reads _vacant_ at 08:00 Stockholm time while the guest is
...
endpoint smaller _and_ more correct.
...
misleading here: the exclusion constraint _is_ the booking rule, so a test against a fake
...
_parsing_ its config and abort with `host not found in upstream` — so the UI container
...
is not, so the config ships as an nginx _template_ using `${NGINX_LOCAL_RESOLVERS}`, taken
